...
abstract class Creature(val hitPoint, val attackDamage: Int){
    def isAlive(): Boolean = this.hitPoint > 0
}

class Hero(_hitPoint: Int, _attackDamage: Int) extends Creatre(_hitPoint, _attackDamage) {
    ...
    override def toString = s"Hero(体力:${hitPoint}, ${attackDamage})"
}

class Monster(_hitPoint: Int, _attackDamage: Int) extends Creature(_hitPoint: Int, _attackDamage: Int) {
